...
def print_command(args: list[str]):
    global print_commands
    if print_commands:
        print(f"\x1b[1;34m$ {shlex.join(args)}\x1b[0m")


def run(args: list[str], properties: dict[str, str]):
    if args[0] == "exit":
        ...
        # print_command(args)
        try:
            check_args(main_sly_kdl.nodes, args[1:], True)
        ...
        print_command(args + [f"{k}={v}" for k, v in properties.items()])
...
def check_args(nodes, args: list[str], check_if_args_handled: bool):
    ...
    subcommand = (
        remaining_args_without_flags[0] if remaining_args_without_flags else "default"
    )
        ...
        if child.name == "$":
            if len(child.args) > 0:
                ...
                for arg in child.args:
                    if arg == "$args":
                        ...
                        cmd.append(arg)
                run(cmd, child.properties)
            else:
                local_variables.update(child.properties)
            ...
            try:
                ...
                if subcommand != "default":
                    raise e
            check_args(child.children, remaining_args, i == len(nodes) - 1)
            remaining_args = []
        elif child.name in flags:
            remaining_args.remove(child.name)
            check_args(child.children, remaining_args_without_flags, False)
